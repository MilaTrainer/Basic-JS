...
// ОБЬЕКТ СОБЫТИЯ 
// (event) - параметр самого собыьтия, обьект события
// у которого есть различные свойсва и методы 
// можно пользлваться им, выводить в консоль, 
// можно посмотеть, что бы видеть на какой точно элемент мы кликнули

divButton1.addEventListener('click', (event) =>{
  console.log(event.target);
  // цель обьекта событий  - целевой элемент
})


// EVENT.STOPpRPPOGATION
// ОТМЕНА ВСПЛЫТИ
// крайне редко
// если кликаю на каой-либо элемент,я не хочу чтобы срабатывали родительсвие обработчики, кроме этого элемента
...
// ПОГРУЖЕНИЕ СОБЫТИЙ- включение (TRUE)
// // advertisement- есть 3 параметра:
// 1. событие 'click',
// 2.передана функция
// 3. и третий параметр обычно равен = fals

// fals - означает - погружение отключено
// обрабатывали сначала все события родителей, а потом погружались внутрь 
// но если мы хотим включить ПОГРУЖЕНИЕ СОБЫТИЙ вместо ВСПЛЫТИЯ
// просто нужно добавить TRUE

divButton1.addEventListener('click',() => {
  console.log('wrapper click');
  // console.log(event.target);
}, true);
// в новой версии JS вместе параметра TRUE - передается:
// обькт, у которого есть свойсво capture 
// {capture: true} = в новой версии JS
// это тоже самое, что написать и TRUe, только современней синтаксис
divButton1.addEventListener('click', () => {
  console.log('привет И');
},{capture: true});
